...
nonnorm.gene.counts_day7 <- as.data.frame(counts(dds, normalized=FALSE))
write.csv(nonnorm.gene.counts_day7, file = "nonnormalized_gene_counts_day7_NO_CBSWP.genotype_clone.temp.csv", row.names = TRUE)
norm.gene.counts_day7 <- as.data.frame(counts(dds, normalized=TRUE))
write.csv(norm.gene.counts_day7, file = "normalized_gene_counts_day7_NO_CBSWP.genotype_clone.temp.csv", row.names = TRUE)
...
#saveRDS(dds, file="DESeq_168.temp.rds")
...
saveRDS(dds, file="DESeq_168_NO_CBSWP.genotype_clone.temp.rds")
#dds <- readRDS("./DESeq_168_NO_CBSWP.genotype_clone.temp.rds")
...
plot.treat.data <- plotPCA(rld, intgroup = c("genotype_clone", "temp"), returnData=TRUE, n=247) ## uses n most variable genes -- not necessarily sig DEGs!
clone <- samples[,4]
...
p1
dev.off()
wdII <- "C:/Users/NJCB/Documents/Purdue/daphnia_RNAseq/figures_tables/02_degs/A7_168_results/"
ggsave(paste(wdII, "168_NO_CBSWP_gc-temp.png", sep=""), plot=p1, units = "in", dpi=1000, height=6, width=8)
ggsave(paste(wdII, "168_NO_CBSWP_gc-temp.svg", sep=""), plot=p1, units = "in", dpi=1000, height=6, width=8)
...
write.csv(as.data.frame(resSig), file="p_05_degs_168hrs_NO_CBSWP_genotype_clone-temp.csv")     ## write CSV with all DEGs, padj < 0.05
#-----------------------------------------------------------------------------------------------------------------------#
...
wdII <- "C:/Users/NJCB/Documents/Purdue/daphnia_RNAseq/figures_tables/02_degs/A7_168_results/"
ggsave(paste(wdII, "168_p0.05_NO_CBSWP_log2fc0_genotype_clone-temp_no_title.png", sep=""), plot=p2, units = "in", dpi=1000, height=8, width=8)
ggsave(paste(wdII, "168_p0.05_NO_CBSWP_log2fc0_genotype_clone-temp_no_title.svg", sep=""), plot=p2, units = "in", dpi=1000, height=8, width=8)
