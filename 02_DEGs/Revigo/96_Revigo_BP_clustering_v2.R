...
dat <- read.table("Revigo_96_results/Revigo_BP_Scatterplot.tsv", header=TRUE, sep="\t")
head(dat)

go <- read.table("./go_96_NO_CBSWP.txt", header=FALSE, sep="\t", na.strings="?", dec=".", strip.white=TRUE)
head(go)
...
d
#looks like 2 maybe for this plot??
...
#Create SVG
wdII <- ("C:/Users/NJCB/Documents/Purdue/daphnia_RNAseq/figures_tables/MS_figures/Fig3_build/Revigo_NO_CBSWP/")
svg(paste(wdII, "96_Revigo_BP_kmeans_clustering_v2.2.svg", sep = ""), height = 6.5, width = 11)
...
annotations <- data.frame(
  index = c(12, 1, 22, 3, 21, 6, 11, 8, 5),  # Indices of points to annotate 
...
legend_values <- c(5, 50, 115)  # Example values from the "Value" column
...
dat5 <- dat5[order(dat5$Cluster, dat5$Frequency), ]
...
write.csv(dat5, paste(wdIII, "96_Revigo_cluster_table_v2.csv", sep=""), col.names = TRUE, row.names = FALSE, quote = FALSE)

#install.packages("writexl")
write_xlsx(dat5, paste(wdIII, "96_Revigo_cluster_table_v2.xlsx", sep=""))
...
wdIII <- c("C:/Users/NJCB/Documents/Purdue/daphnia_RNAseq/figures_tables/MS_figures/TableX-Revigo_cluster/")
dat5 <- read.csv(paste(wdIII, "96_Revigo_cluster_table_v2.csv", sep =""))
...
#looks like some cool things going on in 8 and 9
...
#Create SVG
wdII <- ("C:/Users/NJCB/Documents/Purdue/daphnia_RNAseq/figures_tables/MS_figures/Fig3_build/Revigo_NO_CBSWP/")
svg(paste(wdII, "96_Revigo_BP_kmeans_clustering_v2_cluster_inset_v2.svg", sep = ""), height = 6.5, width = 11)

dat6_filtered <- dat6 %>%
  filter(Cluster %in% c(1, 7)) #1 and 7 have the most clusters
...
annotations_filtered <- dat6_filtered %>%
  filter(str_detect(Name, regex("development|transport", ignore_case = TRUE)))

# Check the filtered results
print(annotations_filtered)
...
for (i in 1:nrow(annotations_filtered)) {
  ...
  # Add the line from the point to the annotation
  segments(point_to_annotate$PC_0, point_to_annotate$PC_1, 
           point_to_annotate$PC_0, point_to_annotate$PC_1 + 0.2)  # Adjust the coordinates as needed
}
